...
use crate::core::gcc::{
    ...
    write_conference_create_request, ClientData, KeyboardLayout, MessageType, ServerData, Version,
};
use crate::core::{per, tpkt, x224};
...
fn domain_parameters(
    max_channel_ids: u32, max_user_ids: u32, max_token_ids: u32, num_priorities: u32, min_throughput: u32,
    max_height: u32, max_mcs_pdu_size: u32, protocol_version: u32,
) -> DomainParameters {
    DomainParameters {
        max_channel_ids,
        max_user_ids,
...
/// Client -- attach_user_request -> Server
/// Client <- attach_user_confirm -- Server
fn attach_user_request() -> u8 { mcs_pdu_header(Some(DomainMCSPDU::AttachUserRequest), None) }
...
fn erect_domain_request() -> RdpResult<Trame> {
    ...
    per::write_integer(0, &mut result)?;
    Ok(trame![mcs_pdu_header(Some(DomainMCSPDU::ErectDomainRequest), None), result.into_inner()])
}
...
impl<S: Read + Write> Client<S> {
    pub fn new(x224: x224::Client<S>) -> Self {
        Client { server_data: None, x224, user_id: None, channel_ids: HashMap::new() }
    }
    ...
    fn write_connect_initial(
        &mut self, screen_width: u16, screen_height: u16, keyboard_layout: KeyboardLayout, client_name: String,
    ) -> RdpResult<()> {
        ...
        let user_data = to_vec(&trame![
            ...
            trame![
                block_header(Some(MessageType::CsSecurity), Some(client_security_data.length() as u16)),
                client_security_data
            ],
            trame![
                block_header(Some(MessageType::CsNet), Some(client_network_data.length() as u16)),
                client_network_data
            ]
        ]);
    ...
    /// ```
    pub fn connect(
        &mut self, client_name: String, screen_width: u16, screen_height: u16, keyboard_layout: KeyboardLayout,
    ) -> RdpResult<()> {
        ...
        match message {
            tpkt::Payload::Raw(mut payload) => {
                ...
                let channel = self
                    ...
                    .ok_or(Error::RdpError(RdpError::new(RdpErrorKind::Unknown, "MCS: unknown channel")))?;
    ...
    pub fn get_user_id(&self) -> u16 { self.user_id.unwrap() }

    /// Getter of the global channel id
    pub fn get_global_channel_id(&self) -> u16 { self.channel_ids["global"] }
}
...
mod test {
    ...
    fn test_connect_initial() {
        ...
        assert_eq!(
            result,
            vec![
                127, 101, 103, 4, 1, 1, 4, 1, 1, 1, 1, 255, 48, 26, 2, 1, 34, 2, 1, 2, 2, 1, 0, 2, 1, 1, 2, 1, 0, 2, 1,
                1, 2, 3, 0, 255, 255, 2, 1, 2, 48, 25, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 0, 2, 1, 1, 2, 2, 4,
                32, 2, 1, 2, 48, 32, 2, 3, 0, 255, 255, 2, 3, 0, 252, 23, 2, 3, 0, 255, 255, 2, 1, 1, 2, 1, 0, 2, 1, 1,
                2, 3, 0, 255, 255, 2, 1, 2, 4, 3, 1, 2, 3
            ]
        );
    }
    ...
    fn test_connect_response() {
        ...
        assert_eq!(
            result,
            vec![
                127, 102, 39, 10, 1, 0, 2, 1, 0, 48, 26, 2, 1, 22, 2, 1, 3, 2, 1, 0, 2, 1, 1, 2, 1, 0, 2, 1, 1, 2, 3,
                0, 255, 248, 2, 1, 2, 4, 3, 1, 2, 3
            ]
        );
    }
}
