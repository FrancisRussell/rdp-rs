...
fn x224_connection_pdu(neg_type: Option<NegotiationType>, mode: Option<u8>, protocols: Option<u32>) -> Component {
...
impl<S: Read + Write> Client<S> {
    ...
    fn new(transport: tpkt::Client<S>, selected_protocol: Protocols) -> Self { Client { transport, selected_protocol } }
    ...
    /// ```
    pub fn connect(
        mut tpkt: tpkt::Client<S>, security_protocols: u32, check_certificate: bool,
        authentication_protocol: Option<&mut dyn AuthenticationProtocol>, restricted_admin_mode: bool,
        blank_creds: bool,
    ) -> RdpResult<Client<S>> {
        Self::write_connection_request(
            &mut tpkt,
            security_protocols,
            Some(if restricted_admin_mode { RequestMode::RestrictedAdminModeRequired as u8 } else { 0 }),
        )?;
    ...
    fn write_connection_request(
        tpkt: &mut tpkt::Client<S>, security_protocols: u32, mode: Option<u8>,
    ) -> RdpResult<()> {
        tpkt.write(x224_connection_pdu(Some(NegotiationType::TypeRDPNegReq), mode, Some(security_protocols)))
    }
    ...
    fn read_connection_confirm(tpkt: &mut tpkt::Client<S>) -> RdpResult<Protocols> {
        ...
        match NegotiationType::try_from(cast!(DataType::U8, nego["type"])?)? {
            NegotiationType::TypeRDPNegFailure => {
                Err(Error::RdpError(RdpError::new(RdpErrorKind::ProtocolNegFailure, "Error during negotiation step")))
            }
            NegotiationType::TypeRDPNegReq => {
                Err(Error::RdpError(RdpError::new(RdpErrorKind::InvalidAutomata, "Server reject security protocols")))
            }
    ...
    pub fn get_selected_protocols(&self) -> Protocols { self.selected_protocol }
